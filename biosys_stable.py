...
def baroni_urbani_buser_coefficient(df):
    ...
    b = 0 #Num species present in only sample A
    c = 0 #Num species present in only sample B
    d = 0 #Num species absent in both samples
    df = df.drop(columns="PrefTaxon")
    for row_num in range(0, df.shape[0]):
        #col 0 is sample A, col 1 is sample B
        count_a = df.iloc[row_num][0]
        if count_a > 1:
            count_a = 1
        count_b = df.iloc[row_num][1]
        if count_b > 1:
            count_b = 1
        if count_a == count_b:
            if count_a == 1:
...
def perform_similarity_checks(sample_list, writer):
    ...
    sim_dict = {"FolderNumber":folder_nums,"Most Similar Sample":sim_samps,"BUB_Co":bub_cos}
...
def save_sample_info(sample_list, writer):
    ...
    header = ("SampleID,SiteID,Region,Area,PRN,Site Name,Sample Date,Barcode,Date of analysis,"
              "BatchNum,Sequence Counts,Pass/Fail,FolderNumber\n")
